...
const App = () => {
  ...
  const checkForRowOfFour = () => {
    for (let i = 0; i < 64; i++) {
      const rowOfFour = [i, i + width, i + 2, i + 3];
      const decidedColor = currentColorArrangment[i];
  ...
  const dragEnd = (e) => {
    ...
    currentColorArrangment[squareBeingDraggeddId] = squareBeingReplaced.getAttribute('src');
    ...
    if( squareBeingDraggeddId && validMove && 
      ...
      currentColorArrangment[squareBeingDraggeddId] = squareBeingDragged.getAttribute('src');
  ...
  }, [checkForColumnOfFour, checkForColumnOfThree, checkForRowOfFour, checkForRowOfThree, moveIntoSquareBelow, currentColorArrangment]);
